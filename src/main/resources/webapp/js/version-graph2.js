function addSpinner(element) {
	...
	var spinner_html = jQuery.parseHTML(
		...
		"</div>");
	$('.revisionGraphVisualisation').append(spinner_html);//jQuery append() 方法在被选元素的结尾插入内容。
	return $('.spinner');
}

// Trim revision number if too long for node,将版本号缩略写出:1234567sdfg  12..dfg
...
function drawGraph(div_selector, _JSON, _showTags) {
   ...
   /* var g;		//d3 graph
    ...
    var svg = d3.select(div_selector).append('div')
        .attr('class','revisionGraphVisualisation')//设置revisionGraphVisualisation的属性值
        .style('overflow', 'scroll')//对其他属性进行更改
        .style('direction','rtl')
        .append('svg');//.append('g');        //添加了2个div
    ...
    d3.select(div_selector).append('div')//
        .attr('id','infos')//设置id值
        .style('display','none'); //设置属性，将信息隐藏

    d3.select('#infos').append('div')//在id为infos的div后面加上一个div
        .attr('id','header')//设置id值
        .html('<h2>Revision</h2>');//在圈圈图下方添加div，设置此div的id为header,来显示被选版本的概述信息
    ...
    //在id为infos的div下方再添加一个div，且加一行字，在加一个div，并将其id设为changesets

  /*  d3.select('#changesets').append('div')
    	.attr('id','addsets')  //添加add的div
    d3.select('#changesets').append('div')
    	.attr('id','deletesets');//添加delete的div     */

	//自己添加的两个框
    d3.select('#changesets').append('div')
        .attr('id','firstRevision')  //添加1.Revision
    d3.select('#changesets').append('div')
        .attr('id','secondRevision');//添加2.Revision

    d3.select(div_selector) //控制时间轴的展开和闭合
    .append('label')    //添加label元素
    .text('Show Axis ')
    .append('input')//添加表单，设置类型为复选框
    .attr('type','checkbox')
    .attr('checked', true) //默认为选中，即，默认为时间轴是展开的
    .on('change', function(){//如果checkbox的选项发生变化，则，进行下列判定与相应
    	if(this.checked){
    		svg.select('g.x.axis').style('display','');
    		//display的默认值是block或inline   //时间轴置为：显示
...
/*
        ...
        svg.on("click", function (d) {
        	revG.selectAll('circle')
        		.style({"opacity": 1, "fill": "white"}); //此处的颜色是控制圆圈的边框
       ...
       // $(window).load(console.log($('g.x.axis')[0].getBoundingClientRect().height));
        ...
        revG.append('circle')
            ...
            .style('stroke-width', function(d){
            	if (d.label != null)  return '5px'; //控制SHAPE的边框
            })
            .on("click", function (d) {
            	//console.log("clicked");
            	//$('circle').css('fill', 'white');
            	//$(this).css('fill', d3.rgb(branches[d.belongsTo].color).brighter(1.5).toString());
            	                                 d3.event.stopPropagation();  //点击一个圆圈之后，停止事件，即停止事件的传播
				click_num ++;
            	revG.selectAll('circle')
            		.style({"opacity": 1, "fill": "white"});
            	d3.select(this)
        			.style({"opacity": .2, "fill": branches[d.belongsTo].color });
            ...
            //自己加上了两个显示两个被选版本的内容的框框
				$("#firstRevision").html("Revision" + " " + trimRevisionNumber(d.revNo) + " " + x(d.d3time.getTime()));
                $("#secondRevision").html();
			...
			.each(function() {   //显示鼠标浮动的Revision的内容
	        	$(this).qtip({
	        		show: 'mouseover',
	        		hide: 'mouseout',
	        		position: {
